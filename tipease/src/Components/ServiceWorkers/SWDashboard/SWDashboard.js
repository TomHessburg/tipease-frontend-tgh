                    ...
                    <Grid container spacing={0}>
                        ...
                        <Grid item lg={4} sm={12} xs={12}>
                            <div className={moduleStyles.tips}>
                                <Paper style={{maxHeight: '230px', overflowY: 'scroll'}} className={classes.paper}>
                                    <Typography variant="h6" gutterBottom>
                                        Tip History
                                    </Typography>
                                    <hr />
                                    {tips.map(tip => {
                                        ...
                                        <div key={tip.id}>
                                            <Typography variant="h6" gutterBottom>
                                                {tip.senderUsername  ? tip.senderUsername : 'no name supplied'}
                                            </Typography>
                                            <Typography variant="body1" gutterBottom>
                                                amount: ${tip.tipAmount}
                                            </Typography>
                                            <Typography variant="body1" gutterBottom>
                                                date: {tip.dateRecieved}
                                            </Typography> 
                                            {tip.senderUsername !== 'not supplied' ?
                                                <div style={{
                                                    ...
                                                    <Icon style={{margin: '0 5px', cursor: 'pointer'}}>email</Icon>
...
const mapStateToProps = state => {
    ...
    let answerPiece = one.split('').reverse().map((num,i) => {
        let num2 = i+1;
        if(num2%3 === 0){
            return ',' + num
        }else{
